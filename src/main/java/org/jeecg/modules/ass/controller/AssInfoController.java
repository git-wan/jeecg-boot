...
import org.jeecg.modules.ass.service.AssInfoService;
...
public class AssInfoController {
	...
	public Result<IPage<AssInfo>> list(AssInfo assInfo, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo, @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize,
									   HttpServletRequest req) {
		Result<IPage<AssInfo>> result = new Result<IPage<AssInfo>>();
		...
		QueryWrapper<AssInfo> queryWrapper = QueryGenerator.initQueryWrapper(assInfo, req.getParameterMap());
		Page<AssInfo> page = new Page<AssInfo>(pageNo, pageSize);
		
		IPage<AssInfo> pageList = assInfoService.page(page, queryWrapper);
...
//		log.info("数据总数：" + pageList.getTotal());
	...
	public Result<AssInfo> add(@RequestBody AssInfo assInfo) {
		Result<AssInfo> result = new Result<>();
		try {
			assInfoService.save(assInfo);
			result.success("添加成功！");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Result<AssInfo> eidt(@RequestBody AssInfo assInfo) {
		Result<AssInfo> result = new Result<>();
		AssInfo AssInfoEntity = assInfoService.getById(assInfo.getId());
		if (AssInfoEntity == null) {
			result.error500("未找到对应实体");
		} else {
			boolean ok = assInfoService.updateById(assInfo);
			// TODO 返回false说明什么？
			if (ok) {
				result.success("修改成功!");
			}
	...
	public Result<AssInfo> delete(@RequestParam(name = "id", required = true) String id) {
		Result<AssInfo> result = new Result<>();
		AssInfo assInfo = assInfoService.getById(id);
		if (assInfo == null) {
			result.error500("未找到对应实体");
		} else {
			boolean ok = assInfoService.removeById(id);
			if (ok) {
				result.success("删除成功!");
			}
		}
	...
	public Result<AssInfo> deleteBatch(@RequestParam(name = "ids", required = true) String ids) {
		Result<AssInfo> result = new Result<>();
		if (ids == null || "".equals(ids.trim())) {
			result.error500("参数不识别！");
		} else {
			this.assInfoService.removeByIds(Arrays.asList(ids.split(",")));
			result.success("删除成功!");
		}
		return result;
	}
	...
	public Result<AssInfo> queryById(@ApiParam(name = "id", value = "示例id", required = true) @RequestParam(name = "id", required = true) String id) {
		Result<AssInfo> result = new Result<>();
		AssInfo assInfo = assInfoService.getById(id);
		if (assInfo == null) {
			result.error500("未找到对应实体");
	...
	public ModelAndView exportXls(HttpServletRequest request, HttpServletResponse response) {
		// Step.1 组装查询条件
		QueryWrapper<AssInfo> queryWrapper = null;
		try {
			String paramsStr = request.getParameter("paramsStr");
			if (oConvertUtils.isNotEmpty(paramsStr)) {
				String deString = URLDecoder.decode(paramsStr, "UTF-8");
				AssInfo assInfo = JSON.parseObject(deString, AssInfo.class);
				queryWrapper = QueryGenerator.initQueryWrapper(assInfo, request.getParameterMap());
				log.info(paramsStr);
				log.info(assInfo.toString());
			}
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
		...
		List<AssInfo> pageList = assInfoService.list(queryWrapper);
		//导出文件名称
	...
	public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				List<AssInfo> listJeecgDemos = ExcelImportUtil.importExcel(file.getInputStream(), JeecgDemo.class, params);
				for (AssInfo jeecgDemoExcel : listJeecgDemos) {
					assInfoService.save(jeecgDemoExcel);
				}
	...
	public AssInfo redisGetJeecgDemo(@PathVariable("id") String id) {
		AssInfo t = assInfoService.getByIdCacheable(id);
